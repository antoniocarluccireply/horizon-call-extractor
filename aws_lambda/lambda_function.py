...
function showResult(rows, downloadUrl){
  ...
  updateScanCounter(rows ?? scanningCounter);
}
...
function updateScanCounter(val){
  scanningCounter = val;
  const el = $("scanCounter");
  if(el) el.textContent = String(scanningCounter);
}
...
function startScanCounter(){
  ...
  scanningTimer = setInterval(() => {
    scanningCounter += 1;
    updateScanCounter(scanningCounter);
    const pct = Math.min(90, 50 + scanningCounter); // keep motion noticeable
    if(!prefersReducedMotion) barSet(pct);
  }, 120);
}

function stopScanCounter(finalVal){
  if(scanningTimer){
    clearInterval(scanningTimer);
    scanningTimer = null;
  }
  if(typeof finalVal === "number" && finalVal >= 0){
    updateScanCounter(finalVal);
  }
}
</script>
</body>
